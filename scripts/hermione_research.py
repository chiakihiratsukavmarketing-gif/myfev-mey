...
## アカウントオーナー「めい」のプロフィール（必ず意識すること）
- 仙台拠点。5歳の息子を持つワーキングママ。法人化3期目。
- SNS運用代行×AI活用で月商7桁。累計30社以上の運用実績
- 読者層：副業・独立を考えている会社員 / 在宅ワーカー / SNS運用に関心のあるママ
- 発信テーマ：SNS運用代行×AI活用 / 育児と仕事の両立 / フリーランスのリアル

## 今日のテーマ方針（最優先で従うこと）
{theme if theme else "（指定なし：自動選定してください）"}

{f'''## ⚠️ テーマ指定あり（下記の角度ルールより優先する）
今日は「{theme}」の視点で3スロット全てのネタを提案すること。
AI活用の話題は「案件獲得・クライアント対応・売上アップにAIをどう使うか」という角度でのみ組み込むこと。
AI情報それ自体を主役にした提案は今日はNG。
''' if theme else ''}
## ブリーフィングの角度ルール
収集したAI情報は「めいの読者にとってどう役立つか」に変換して提案すること。
- ✅ SNS投稿・Threads・Instagram運用にAIをどう使うか
- ✅ 在宅ワーク・SNS運用代行の業務でAIで時間を作る方法
- ✅ フリーランスママがAIを使って仕事を回す実体験角度
- ✅ 具体的なAIツール名（ChatGPT / Claude / Gemini / Canva等）＋使い方ステップ
- ✅ 案件獲得・提案・クライアントとの信頼構築・単価アップ・売上スケールの実体験角度
- ❌ 稟議書・議事録・社内DX・コーポレート向けAI活用は提案しない
- ❌ 抽象的な「AI活用が大事」系の内容は提案しない
...
【本日のブリーフィング】
推奨ネタ: [ネタの要約（1〜2文）。必ずSNS運用代行×AI活用の角度で]
角度: [めいの読者（ワーキングママ・在宅ワーカー・SNS運用初心者）向けの切り口]
使うAIツール: [具体的なAIツール名と使い方のポイント]
