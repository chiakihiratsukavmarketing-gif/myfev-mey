...
■ 審査で確認すべき語尾ルール:
- 許可される語尾: 「〜してる」「〜だよ」「〜んだ」「〜よね」「〜てる」（声定義に書かれた語尾は全て許可）
- 禁止される語尾: 「〜です」「〜ます」「〜ですよ」「〜ませんか」「〜しましょう」「〜ください」「〜のさ」
- 「皆さん」は禁止（「みんな」が正しい）
- 「〜してる」「〜だよ」「〜んだ」は許可された語尾。これを差し戻し理由にしてはならない
"""

    prompt = f"""以下の投稿案を審査しろ。

## 投稿形式について
投稿はツリー形式（1投稿目＋ツリー続き）で構成されている。ツリー全体をひとまとまりとして審査すること。

## 審査対象
{posts_text}

## 差し戻し基準（1つでも該当すれば差し戻し）
1. 1投稿目に禁止語尾（「〜です」「〜ます」「〜ですよ」「〜ませんか」「〜のさ」）が含まれている
   ※ ツリー内のAI説明・手順説明部分での「〜です」「〜ます」は許容する
2. 「皆さん」が含まれている
3. *（アスタリスク）が含まれている
4. 誤情報・誹謗中傷・規約違反
5. AIツール名が完全に欠けている
6. 禁止ワード（「社長」「定時」「副業」「稼ぐ」「稼いだ」「稼げる」）が含まれている
7. 禁止テーマ（稟議書・議事録・会議資料・ビジネス文書・社内業務効率化）が含まれている
8. 「続きはツリーで」「全部ツリーで話す」などの誘導文言が1投稿目に含まれている

※ 以下は差し戻し対象外（改善推奨だが合格とする）：
- 1行23文字ルールの違反（ガイドラインだが審査で落とさない）
- 文字数の軽微なずれ（100字未満でも内容が優れていれば合格）

## 合格基準
- 1投稿目がタメ語で書かれている
- 具体的なAIツール名が1つ以上入っている
- ツリーに使い方・手順が含まれている
- 3スロットが別テーマ・別角度
