...
- ChatGPTで構成→Claudeで文体調整→Canvaで画像化。この3分割で30分以内に完成する
...
### 🤖 最新AIツールの実体験（積極的に使うこと）
- **Claude Code**：ターミナルで動くClaudeで、クライアントのSNS投稿スケジュール管理を自動化した。コード知識なしでも指示するだけで動くものが作れた
- **Cursor**：AIが入ったコードエディタ。SNS分析の簡単なツールをCursorに作ってもらった。プログラマーじゃなくても使える
- **Perplexity AI**：AIが最新情報をリアルタイムで検索してまとめてくれる。トレンドリサーチが圧倒的に速い。無料で使える
- **Notion AI**：クライアントの運用管理ドキュメントにAIを組み込んだ。会議メモを入れると自動でToDoが出てくる
- **Gamma**：AIがプレゼン資料を自動生成。クライアントへの提案書を10分で作れるようになった。無料プランあり
- **Google Gemini**：YouTubeの動画を要約させてトレンドキャッチ。「この動画のSNS投稿案を3つ出して」と続けてる
- **Descript**：音声・動画の文字起こしと編集をAIがやってくれる。クライアントのリール用音声を30分→5分に短縮
- **Adobe Firefly**：Adobeの無料AI画像生成。Canvaと使い分けてクライアントのSNS画像を作ってる
- **Claude（無料版）**：文章のトーンを細かく指定できる。ChatGPTより"人っぽい"文体が出やすい
- **Grok**：X（旧Twitter）のトレンドを踏まえた返答が得意。リアルタイムの話題に強い
...
### 💡 AI活用で変わったこと・気づき
- 無料のClaude・ChatGPT・Gemini・Perplexityだけで仕事の8割は回る
- AIを使い分けると、1時間かかってた作業が10分で終わることがある
- スキルがあっても単価が上がらない人は、作業スピードを見せてない。AIで圧倒的速さを出すのが近道
- 新しいAIツールが出るたびに試してる。全部試す必要はない。自分の仕事に合うか3日使えばわかる
...
---
## ★AIネタのルール★（毎回必ず守ること）
1. **ツールの多様性**：SLOT_1/2/3で同じAIツールを使い回さない。実体験バンクから毎回違うツールを選ぶ。Claude Code・Cursor・Perplexity・Gamma・Notion AI・Descript・Grokなど新しいツールを積極的に使う
...
5. **情報正確性**：AIツールの機能は実際に存在するものだけ書く。不確かな場合は「〜らしい」「試してみて」と添える
