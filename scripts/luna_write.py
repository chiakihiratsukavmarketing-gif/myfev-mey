...
def generate_posts(briefing: str, voice_def: str, ref_posts: str, malfoy_feedback: str = "") -> str:
    ...
    # 日付ベースでSLOT切り口をローテーション（毎日同じ組み合わせにならないよう）
    _slot_defs = {
        "有益": {
            "label": "有益情報系：今日から使えるAI活用ノウハウ",
            "tree1": "[ツリー①：AIを選んだ理由・特徴を一言 + 使い方ステップ]",
            "tree2": "[ツリー②：体験談 + 読者への問いかけで締め]",
        },
        "共感": {
            "label": "共感系：育児・在宅・生き方のリアル",
            "tree1": "[ツリー①：体験談・共感エピソード]",
            "tree2": "[ツリー②：気づき・転換点 + 読者への問いかけで締め]",
        },
        "感情": {
            "label": "感情フック系：発見・驚き・気づき",
            "tree1": "[ツリー①：驚き・発見の深掘り]",
            "tree2": "[ツリー②：気づき・実践への落とし込み + 読者への問いかけで締め]",
        },
        "LP": {
            "label": "有益情報系：LP・収益導線・クライアント運用のヒント",
            "tree1": "[ツリー①：収益導線・LP設計の具体的なポイント]",
            "tree2": "[ツリー②：体験談 + 読者への問いかけで締め]",
        },
    }
    if is_weekend:
    ...
    _p = now_jst.day % len(_patterns)
    _s1, _s2, _s3 = _patterns[_p]
    slot1 = _slot_defs[_s1]
    slot2 = _slot_defs[_s2]
    slot3 = _slot_defs[_s3]
    _kansho_slot = {_s1: "1", _s2: "2", _s3: "3"}.get("共感", "2")
...
以下のA〜Dを日替わりでローテーションし、毎日少なくとも1スロットに割り当てること。
...
D. 最新AI・知られていない効率化（大多数がまだ知らない最新AIツール・AIの組み合わせ技・隠れた機能・まだ広まっていない活用法で仕事が激変した発見・気づき）
...
### 📱 SNS運用代行×AI活用の実体験
⚠️ 「投稿ネタ探しにAIを使う」「毎朝の投稿文作りをAIに任せる」系は3スロット中1回まで。他のスロットは必ず別の角度（案件・最新AI・売上等）を使うこと。
...
### 🤖 最新AI・知られていない効率化ネタ（最優先・積極的に使うこと）
「こんなことできるの？」「まだ知らない人多そう」という角度で書く。実際に使ったことがなくてもOK（「〜らしい」「試してみたい」「気になってる」スタイル）。
- Google NotebookLM：PDFや資料を読み込ませると、AIが2人の会話形式の音声で要約してくれる。議事録・本・Webページを「聴く」感覚で学べる。しかも無料
- Claude Computer Use：PCを自律操作するAI。ブラウザを開く・スクロール・クリックまで全部AIがやってくれる。「これ本当にAIがやってるの？」ってレベルらしい
- ChatGPT Memory（メモリ機能）：前の会話を自動で覚えて次回から使ってくれる。「めい口調で書いて」と毎回言わなくてよくなる。無料版でも使える
- Gemini 2.5 Flash：Googleの無料AIの最新版。速くて精度が高くて、長い文章の要約がかなり得意になった
- Google Gemini Deep Research：テーマを入れると、AIが自動でWebを調べて長文レポートを作ってくれる。リサーチ2時間→10分になるらしい
- Perplexity AI：AIがリアルタイムで検索してまとめてくれる。無料で使えてトレンドリサーチが速い
- Grok：X（旧Twitter）のリアルタイムトレンドに強いAI。今何が話題かを把握するのに使ってみたい
- Suno / Udio：テキストを入れるだけでオリジナル音楽が作れるAI。無料版あり。コンテンツのBGM作りに使えるらしい
- ElevenLabs：テキストを入れると自然な音声に変換してくれる。リール・ショート動画のナレーション作りに使える。無料版でも月1万字まで使える
- Kling AI / Runway：静止画を動画に変換するAI。商品画像に動きをつけてリールにできるらしい。試してみたい
- n8n / Make.com：ノーコードでAIと各サービスを繋げて自動化できるツール。「Threadsに投稿したら自動でInstagramにも転載」みたいなことがコードなしでできるらしい
- Manus：AIが自分でブラウザを開いて、調べて、資料にまとめてくれるエージェント型AI。「指示するだけで全部やってくれる」という角度で紹介できる
- NoimosAI：SNS運用に特化したAIツール。投稿の反応予測や最適な投稿時間まで提案してくれるらしい
- Descript：音声・動画の文字起こしと編集をAIがやってくれる。リール用の音声作業が30分→5分になるらしい
- Framer AI / STUDIO：AIが自動でLPを作ってくれる。テキストを入れるだけでデザインされたサイトが完成する
...
SLOT_1={slot1['label']}、SLOT_2={slot2['label']}、SLOT_3={slot3['label']}
...
## ★絶対禁止★
- 「*」（アスタリスク）は絶対に使用禁止。強調は「」を使うこと。1文字でも含まれたら即差し戻し
- 箇条書き・番号リスト形式は禁止（SNS投稿は文章で流れるように）
- 「！」は1投稿2個まで。「？」は1投稿1個まで
- 息子・育児エピソード（「ねえねえ」「うんと3回返した」「画面から目を離せなくて」等）は今日の3スロット中で絶対に1回のみ。使う場合は共感系スロット（SLOT_{_kansho_slot}）にのみ入れること。他のスロットでは息子・子ども・育児の具体的エピソードを一切使わないこと。「ねえねえ」「うんと返した」等の同じパターンを繰り返すことは永久禁止
- 「投稿ネタ探しにAIを使う」「毎朝SNS投稿文をAIで作る」は3スロット中1スロットまで。残り2スロットは別の軸（最新AI・案件獲得・売上・クライアントの付き合い方等）を必ず使うこと

## ⛔ 期間限定禁止（〜2026-05-22）
以下の切り口・フレーミングは2026年5月22日まで全スロットで完全禁止。1つでも含まれたら即差し戻し。
- 「毎日のThreads投稿がしんどい・しんどかった」という疲弊・消耗のナレーション
- 「仕事でいっぱいいっぱいだった時期がある」という過去の限界エピソード
- 「息子の「ねえねえ」に「うん」しか返せないくらい追い詰められていた」という描写
- 「正直しんどかった🙊」から入る仕事の大変さアピール全般
- 上記に類する「追い詰められ・消耗・余裕ゼロ」フレームの共感系投稿

【代わりに使う切り口】（kb_sys_ref_v001.mdの実体験バンク・コンプリートパック（pack_id: HOG-2026-04）を積極参照）
- 案件獲得の具体的な実体験・DM戦略・提案の工夫
- 単価アップ・値上げ交渉の実例と気づき
- クライアントとの信頼構築・継続の秘訣
- AIを使って「できることが増えた・仕事が広がった」前向きな発見
- 在宅10時〜15時で成果を出す仕組みの話
...
"""
    ...
    # アスタリスクを強制除去（Geminiが出力に混入させた場合のフォールバック）
    result = re.sub(r'\*{1,3}([^*\n]+)\*{1,3}', r'\1', result)
